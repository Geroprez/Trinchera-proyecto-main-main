...
public class InteraccionNPC : MonoBehaviour
{
    ...
    private GestionFinal gestionFinal; // Referencia al script de Gestión de Finales
    ...
    private void Start()
    {
        ...
        // Obtiene el componente GestionFinal (asegúrate de tenerlo asignado en el Inspector)
        gestionFinal = FindObjectOfType<GestionFinal>(); // Busca el script GestionFinal en la escena
    }
    ...
    private void Update()
    {
        ...
        {
            // Iniciamos la interacción con el NPC
            estaInteraccionando = true;
            interaccionRealizada = true;
            movimientoJugador.DeshabilitarMovimiento(tiempoInteraccionNPC);
            movimientoJugador.ActivarAnimacionInteraccionConNPC(); // Activar animación de interacción
            InteraccionObstaculo.IncrementarNPCContador(); // Incrementa el contador en InteraccionObstaculo (si lo usas aquí)
            
            // Incrementamos el contador de NPCs en el script GestionFinal
            if (gestionFinal != null)
            {
                gestionFinal.IncrementarNPCs(); // Llama al método que aumenta el contador en GestionFinal
            }
